...
{
    public class ApiManagementDashboardScopeHandler(
        ILogger logger,
        ...
        ICacheServiceFactory cacheServiceFactory) : IDashboardScopeHandler
    {
        ...
        public string Scope => DashboardScopes.ApiManagement;
        ...
        public async Task<Result<DashboardResponseBase>> HandleAsync(GetDashboardQuery request, CancellationToken cancellationToken)
        {
            ...
            var cacheKey = $"dashboard:{DashboardScopes.ApiManagement}";
            var cachedDashboard = await _cacheService.GetAsync<ApiManagementDashboardDto>(cacheKey, cancellationToken);
            if (cachedDashboard != null)
            {
                return Result<DashboardResponseBase>.Success(cachedDashboard);
            }
            ...
            {
                return Result<DashboardResponseBase>.Failure(clustersResult.ErrorCode, clustersResult.ErrorMessage);
            }

            var routesResult = await routesTask;
            if (!routesResult.IsSuccess)
            {
                return Result<DashboardResponseBase>.Failure(routesResult.ErrorCode, routesResult.ErrorMessage);
            }
            ...
            {
                return Result<DashboardResponseBase>.Failure(totalClustersResult.ErrorCode, totalClustersResult.ErrorMessage);
            }

            var totalRoutesResult = await totalRoutesTask;
            if (!totalRoutesResult.IsSuccess)
            {
                return Result<DashboardResponseBase>.Failure(totalRoutesResult.ErrorCode, totalRoutesResult.ErrorMessage);
            }

            var apiProductsResult = await apiProductsTask;
            if (!apiProductsResult.IsSuccess)
            {
                return Result<DashboardResponseBase>.Failure(apiProductsResult.ErrorCode, apiProductsResult.ErrorMessage);
            }
            ...
            var dashboard = new ApiManagementDashboardDto
            {
                Summary = new ApiManagementDashboardSummaryDto
                {
                    Clusters = totalClustersResult.Data,
                    Routes = totalRoutesResult.Data,
                    Products = productsCount,
                },
                Charts = new ApiManagementDashboardChartsDto
                {
                    GrowthTrend = new ApiManagementGrowthTrendChartDto
                    {
                        Labels = monthLabels,
                        Datasets =
                        [
                            new ApiManagementDatasetDto { Label = "Clusters", Data = clusterGrowth },
                            new ApiManagementDatasetDto { Label = "Routes", Data = routeGrowth },
                        ]
                    },
                    ClusterStatusDistribution = new ApiManagementStatusDistributionChartDto
                    {
                        Labels = ["Active", "Inactive"],
                        Counts = [activeClusters, inactiveClusters],
                    },
                    MonthlyRouteActivity = new ApiManagementMonthlyCountsChartDto
                    {
                        Labels = monthLabels,
